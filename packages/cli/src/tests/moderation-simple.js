...
import { WalletAuth, PeerIdentityManager, KickManager, BanManager, AdminManager, ModerationEnforcer, } from '@accord/core';
async function main() {
    console.clear();
    console.log('╔════════════════════════════════════════════════════════════╗');
    ...
    const OWNER_ADDRESS = '0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb1';
    const ADMIN_ADDRESS = '0x1234567890123456789012345678901234567890';
    const TARGET_ADDRESS = '0xabcdefabcdefabcdefabcdefabcdefabcdefabcd';
    ...
    identityManager.myAddress = OWNER_ADDRESS;
    ...
    const moderationEnforcer = new ModerationEnforcer(kickManager, banManager, adminManager, connectionManager, identityManager, { autoEnforce: true });
    ...
    identityManager.peerIdentities.set(targetPeerId, {
        address: TARGET_ADDRESS,
        peerId: targetPeerId,
        verified: true,
        verifiedAt: Date.now(),
    });
    identityManager.peerIdentities.set(adminPeerId, {
    ...
    console.log('─────────────────────────────────────────────────────────────\n');
    const kickDuration = 5 * 60 * 1000;
    console.log(`Kicking peer: ${targetPeerId}`);
    console.log(`Reason: "Testing kick system"`);
    console.log(`Duration: 5 minutes\n`);
    const kickEntry = await moderationEnforcer.kickPeer(targetPeerId, 'Testing kick system', kickDuration);
    ...
    const banEntry = await moderationEnforcer.banPeer(targetPeerId, 'Testing ban system');
...
//# sourceMappingURL=moderation-simple.js.map